function [FIM,d1, flag] = calculate_complete_fim_3d(params, valid_walls)
    ...
    for i = 1:length(params.walls)
        % check if i is in valid_walls
        if ~ismember(i, valid_walls)
            continue;
        end
   ...
   %function G = calculate_gradient_matrix_3d(W, F, aTx, aRx, exp_k, X, alpha, phi_rx, phi_tx, theta_rx, theta_tx, params)
    ...
    angleRx_Res = 2/params.Nrx_x;

    delta_delay = abs(tau1-[tau0 reflected_paths.tau])<delay_Res; % 1 = not resolvable, 0 = resolvable
    ...
    % addition == 3 -> Not resolvable on all domains
    % deltas = 1 -> not resolvable. It will be = 1 if any of the paths is
    % not resolvable from the target path (summation is 1 or more)

    
    deltas = delta_delay + delta_angleTx + delta_angleRx;
    if any(deltas(:) == 3), deltas=1; else, deltas=0; end
    
%     fprintf('Target position: [%.2f, %.2f, %.2f]\n', params.pT(1), params.pT(2), params.pT(3));
%     
%     fprintf('%.3e ', deltas);
% 
%     fprintf('\n');
%     fprintf('Delta delay values (|tau1 - tau|): ');
%     fprintf('%.3e ', delta_delay);
%     fprintf('%.3e ',abs(tau1-[tau0 reflected_paths.tau]));
%     fprintf('\n');
%     fprintf('Delta angleTx (|sin(phi_Tx1) - sin(phi_Tx)|): ');
%     fprintf('%.3e ', delta_angleTx);
%     fprintf('%.3e ', abs(sin(phi_Tx1)-[sin(phi_Tx0) sin([reflected_paths.phi_Tx])]));
%     fprintf('\n');
%     fprintf('Delta angleRx (|sin(phi_Rx1) - sin(phi_Rx)|): ');
%     fprintf('%.3e ', delta_angleTx);
% 
%     fprintf('%.3e ', abs(sin(phi_Rx1)-[sin(phi_Rx0) sin([reflected_paths.phi_Rx])]));
%     fprintf('\n');
%     fprintf('----------------------------------------\n\n');
